...
| ENG-11 | `end_encounter` command + teardown | M3 engine track (deferral-verify §14) | when encounter teardown is needed | doc-only | Deferred | Arch §4 lists `EndEncounterCommand`; `commands/types.ts` has none. `reaction`/`EncounterState` lingers post-combat (harmless — nothing reads it off-encounter; only cleared by overwrite). |
