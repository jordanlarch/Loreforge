...
(AGENTS.md, M6 gate) but is not built — there is no `/tutorial` route.

| ID | Item | Deferred-to | Tracking | Status | Notes |
|---|---|---|---|---|---|
| TUT-1 | "Lantern's Last Flicker" tutorial E2E (scripted scene state machine, first-time tooltips, graduation modal, replay) | P5 / **M6** | doc-only | Missing | The capstone onboarding green-path; gates strict open-beta/GA. Tutorial TTS/voiceover polish and the multiplayer onboarding flow are separate v1.5 cuts (`tutorial-adventure.md` §§9.3, 11.1). |
