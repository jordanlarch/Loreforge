...
| MEM-5 | Live-turn RAG injection into the AI-GM prompt (5-part context assembler + rerank) | data-sources §6 | P5 | #139 | Done | **Shipped (#140):** owner-scoped `retrieveSimilar` over `realm_entity` embeddings injected into the live AI-GM narration prompt as a background "world knowledge" block (`ws-server/world-knowledge.ts` best-effort + similarity floor; `narration.ts` `knowledge[]`; `getCampaignOwnerId`; wired into player-turn + check-outcome paths). Env-gated on `OPENAI_API_KEY`; no-ops offline so narration is unchanged. Tracer slice — the full 5-part assembler + multi-category rerank (recency/pinning/cross-link) still layers on once rolling summary (MEM-3) + recaps (MEM-4) source types exist. |
