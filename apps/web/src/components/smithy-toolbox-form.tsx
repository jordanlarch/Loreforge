...
function SmithyFearStressToolboxForm({
  initial,
  entryId,
  onSaved,
  onCancel,
  ...
  const fearStress: FearStressDefinition =
    initial?.definition.kind === "fear_stress"
      ? initial.definition
      : {
          kind: "fear_stress",
          id: "new-fear-stress",
  ...
  const [name, setName] = useState(initial?.name ?? fearStress.name);
  const [description, setDescription] = useState(
    initial?.description ?? fearStress.description,
  );
  const [saveAbility, setSaveAbility] = useState(
    fearStress.save?.ability ?? "wis",
  );
  const [saveDc, setSaveDc] = useState(
    fearStress.save ? String(fearStress.save.dc) : "",
  );
  const [saveOnSuccess, setSaveOnSuccess] = useState<
    NonNullable<FearStressDefinition["save"]>["onSuccess"]
  >(fearStress.save?.onSuccess ?? "negates");
  const [effects, setEffects] = useState(fearStress.effects?.join("\n") ?? "");
  const [duration, setDuration] = useState(fearStress.duration ?? "");
  const [error, setError] = useState<string | null>(null);
      ...
      fearStress: {
        save:
          !Number.isNaN(dc) && dc > 0
            ...
            : undefined,
        effects: effectList.length > 0 ? effectList : undefined,
        duration: duration.trim() || undefined,
      },
    };
  }
  ...
  return (
    ...
    >
      <h3 className="font-display text-lg">
        {entryId ? "Edit fear/stress" : "Forge a fear/stress effect"}
      </h3>
      <p className="text-xs text-lore-muted">
        Complex PDF mechanics (variable DCs, DM discretion, multi-phase fear) can
        be captured in effect lines and duration prose.
      </p>
      ...
      <Field label="Effects (one per line)">
        <textarea
          className={`${inputClass} min-h-[100px]`}
          value={effects}
          onChange={(e) => setEffects(e.target.value)}
          placeholder="Frightened on a failed save."
        />
      </Field>

      <Field label="Duration / special rules">
        <textarea
          className={`${inputClass} min-h-[60px]`}
          value={duration}
          onChange={(e) => setDuration(e.target.value)}
...
function SmithyCurseToolboxForm({
  initial,
  entryId,
  onSaved,
  onCancel,
  ...
  const curse: CurseDefinition =
    initial?.definition.kind === "curse"
      ? initial.definition
      : {
          kind: "curse",
          id: "new-curse",
  ...
  const [name, setName] = useState(initial?.name ?? curse.name);
  const [description, setDescription] = useState(
    initial?.description ?? curse.description,
  );
  const [contagion, setContagion] = useState(curse.contagion ?? "");
  const [saveAbility, setSaveAbility] = useState(
    curse.save?.ability ?? "con",
  );
  const [saveDc, setSaveDc] = useState(
    curse.save ? String(curse.save.dc) : "",
  );
  const [saveOnSuccess, setSaveOnSuccess] = useState<
    NonNullable<CurseDefinition["save"]>["onSuccess"]
  >(curse.save?.onSuccess ?? "negates");
  const [effects, setEffects] = useState(curse.effects?.join("\n") ?? "");
  const [recovery, setRecovery] = useState(curse.recovery ?? "");
  const [error, setError] = useState<string | null>(null);
  ...
  function buildPayload() {
    ...
    return {
      name: name.trim(),
      topic: "curse" as const,
      description: description.trim(),
      source: initial?.source ?? ("original" as const),
      copiedFromSlug: initial?.copiedFromSlug ?? undefined,
      curse: {
        contagion: contagion.trim() || undefined,
        save:
          !Number.isNaN(dc) && dc > 0
            ? {
                ability: saveAbility,
  ...
  return (
    ...
    >
      <h3 className="font-display text-lg">
        {entryId ? "Edit curse" : "Forge a curse"}
      </h3>
      ...
      <Field label="Contagion / spread">
        <textarea
          className={`${inputClass} min-h-[60px]`}
          value={contagion}
          onChange={(e) => setContagion(e.target.value)}
          placeholder="How the curse or contagion spreads"
        />
      </Field>
      ...
      <Field label="Effects (one per line)">
        <textarea
          ...
          value={effects}
          onChange={(e) => setEffects(e.target.value)}
          placeholder="Blinded until the contagion ends."
        />
      </Field>

      <Field label="Recovery / removal">
        <textarea
          className={`${inputClass} min-h-[60px]`}
          value={recovery}
      ...
      <div className="flex flex-wrap gap-2">
        <button
          ...
          className="rounded border border-lore-accent bg-lore-accent-dim px-4 py-2 text-sm text-lore-text disabled:opacity-50"
        >
          {pending ? "Saving…" : entryId ? "Save changes" : "Forge curse"}
        </button>
        {onCancel ? (
          <button
            type="button"
