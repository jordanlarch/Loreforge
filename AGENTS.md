...
- **Code status (Jun 2026)** — **P0–P4 substantially complete; P5 (Tutorial + Memory) largely shipped early.** Milestones: **M1–M4** at vertical-slice/tracer depth; **M5 "First Campaign" ~complete** (see historical snapshot `docs/archive/03-m5-completion-plan.md`); **M6 Tutorial E2E shipped** (TUT-1 #169–#178, user verified). All 7 rich generators shipped (Settlement tab depth partial). Campaign prep is a **7-tab workspace** (Overview / Map / Locations / Party / Quests / Notes / Settings — Map = overworld grid, CAMP-7 partial) + a unified Live Play shell. Live Play substantial (chat/HUD/combat/enemy-AI/reactions/right party-rail/top-bar/scene-transitions). Memory tier MEM-1–8 Done. **SRD 5.1→5.2 audit (AUDIT-0–9) complete** — see `docs/srd-version-audit.md`; **feature/fidelity coverage map** in `docs/srd-feature-coverage-audit.md` (`SRD-FID-*` rows in `deferrals.md` §12; incongruence bugs = issues #310–#324). **Gameplay Toolbox Live Play v1 complete** (traps → fear, #292–#305); **Smithy toolbox removed** (#306). **GRILL-EXPLORATION complete** (Q1–Q9; Slices 1–3 shipped #307–#309). **For the live backlog / active frontier and all status, `docs/deferrals.md` is the single source of truth.** Branch **`main`**. Package manager is **npm** (not pnpm).
