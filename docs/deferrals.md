...
**#104 partial** (Live Play reactions — AI-reactor opportunity attacks: a `runEnemyReactions` WS-server driver auto-takes the engine `opportunity_attack` for every AI-controlled reactor when a PC flees its reach, with enriched resolution rows + narration; Ready actions + pacing controls remain — issue stays open).
